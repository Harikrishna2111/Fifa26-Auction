...
const Auction = () => {
  ...
  const [currentBid, setCurrentBid] = useState(85); // Default start price
  const [highestBidder, setHighestBidder] = useState("No Bids Yet");
  const [auctionStatus, setAuctionStatus] = useState("LOBBY");
  ...
  // (Keeping your existing dummy data arrays for display)
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      // Join specific auction room (Hardcoded ID for now)
      newSocket.emit('join_auction', { auction_id: 1, user_id: 999 }); 
    });
    ...
    newSocket.on('sync_state', (data) => {
       if(data.time) setTimer(data.time);
  ...
  const handlePlaceBid = (increment) => {
    ...
    const newAmount = increment === 'CUSTOM' 
      ? parseFloat(customBidAmount) 
      : currentBid + increment;

    socket.emit('place_bid', {
      auction_id: 1,
      player_id: 1, // Currently hardcoded to Mbappe (Player 1)
      team_id: 1, // Hardcoded User Team ID
      user_name: "MyManagerProfile", // Should come from Auth Context
      amount: newAmount
    });
  };
  ...
  // --- COMPARE MODAL & BOUGHT MODAL (Copied from your code - Unchanged for brevity, assumed existing) ---
  const CompareModal = () => {
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden backdrop-blur-sm flex-1 flex flex-col shadow-2xl">
            ...
            <div className="p-6 flex-1 bg-black/40">
              ...
              <div className="flex gap-2">
                ...
                <div className="flex-1 h-12 bg-[#ff4d4d]/10 border border-[#ff4d4d]/30 rounded flex items-center justify-center text-[#ff4d4d] font-bold text-xl">00</div>
                <div className="flex-1 h-12 bg-[#ff4d4d] border border-[#ff4d4d] rounded flex items-center justify-center text-white font-bold text-xl">
                    {/* DYNAMIC TIMER */}
          ...
          <div className="flex-1 bg-white/5 border border-white/10 rounded-xl backdrop-blur-sm flex flex-col items-center justify-center relative overflow-hidden shadow-2xl">
            ...
            <div className="absolute left-1/2 top-6 -translate-x-1/2 flex items-center gap-8 bg-black/40 border border-white/10 px-6 py-2 rounded-full backdrop-blur-xl shadow-lg z-20">
              ...
              <div className="flex items-center gap-3">
                {/* PAUSE / RESUME BUTTON */}
                <button 
                  onClick={() => handleAdminAction(auctionStatus === 'PAUSED' ? 'RESUME' : 'PAUSE')}
                  className="size-9 rounded-full bg-yellow-500/10 border border-yellow-500/30 text-yellow-500 flex items-center justify-center hover:bg-yellow-500/20 hover:scale-110 transition-all">
                  <span className="material-symbols-outlined text-lg">
                    {auctionStatus === 'PAUSED' ? 'play_arrow' : 'pause'}
                  </span>
                </button>
                {/* END AUCTION BUTTON */}
            ...
            <div className="text-center z-10">
              ...
              {/* DYNAMIC BID PRICE */}
              <h1 className="text-[120px] font-bold leading-none tracking-tighter text-[#ffd700] italic mb-2">
                ${currentBid}<span className="text-6xl not-italic ml-2">M</span>
              </h1>
              <div className="flex items-center justify-center gap-3">
                ...
                {/* DYNAMIC BIDDER NAME */}
                <p className="text-[#ffd700] font-medium uppercase tracking-widest text-sm">Bidder: <span className="text-white">{highestBidder}</span></p>
              </div>
            </div>
            <div className="mt-12 w-full max-w-xl px-12 z-10">
              <div className="grid grid-cols-3 gap-4 mb-4">
                {/* LIVE BID BUTTONS */}
                <button onClick={() => handlePlaceBid(5)} className="h-16 bg-[#39ff14] text-black font-black text-xl italic rounded hover:scale-105 transition-transform">+$5M</button>
                <button onClick={() => handlePlaceBid(10)} className="h-16 bg-[#39ff14] text-black font-black text-xl italic rounded hover:scale-105 transition-transform">+$10M</button>
                <button onClick={() => handlePlaceBid(20)} className="h-16 bg-[#39ff14] text-black font-black text-xl italic rounded hover:scale-105 transition-transform">+$20M</button>
              </div>
              <div className="flex gap-4">
                {/* LIVE CUSTOM BID */}
                <input 
                  className="flex-1 bg-black/50 border border-white/20 rounded px-6 font-bold text-lg text-white" 
                  placeholder="Custom bid..." 
                  type="number"
                  value={customBidAmount}
                  onChange={(e) => setCustomBidAmount(e.target.value)}
                />
                <button onClick={() => handlePlaceBid('CUSTOM')} className="h-14 w-32 bg-[#39ff14] text-black font-black text-xl italic rounded hover:scale-105">BID</button>
        ...
        <aside className="w-[340px] flex flex-col gap-6">
          <div className="h-1/3 bg-white/5 border border-white/10 rounded-xl backdrop-blur-sm flex flex-col overflow-hidden">
            <div className="p-3 border-b border-white/10 bg-black/20 font-bold text-xs uppercase tracking-widest flex justify-between items-center">
              ...
              <span className={`text-[9px] px-2 rounded font-bold ${auctionStatus === 'PAUSED' ? 'bg-yellow-500/20 text-yellow-500' : 'bg-[#39ff14]/20 text-[#39ff14]'}`}>
                {auctionStatus === 'PAUSED' ? 'PAUSED' : 'REAL-TIME'}
              </span>
            </div>
