  ...
  return (
    <div
      ...
      className={`
        ...
        cursor-grab active:cursor-grabbing overflow-hidden transition-all duration-150 select-none
...
const Formation_settings = () => {
  ...
  useEffect(() => {
    ...
    const avg = pitchPlayers.length ? Math.floor(total / 11) : 0;
    const coords = formations[currentFormation];
    let chem = 0;
    pitchPlayers.forEach((p, i) => {
      if (p && coords[i] && p.pos === coords[i].role) chem += 3;
      else if (p) chem += 1;
    });
    setSquadRating(avg);
    setSquadChemistry(`${Math.min(chem, 33)}/33`);
  }, [pitchPlayers, currentFormation]);

  // --- DRAG HANDLERS ---

  const handleDragStart = (e, type, index, player) => {
    // We use a simple object string for data transfer to ensure compatibility
    e.dataTransfer.setData("dragPayload", JSON.stringify({ type, index }));
    e.dataTransfer.effectAllowed = "move";
    setDraggedInfo({ type, index });
  };

  const handleDragOver = (e, type, index) => {
    e.preventDefault();
    e.dataTransfer.dropEffect = "move";
    if (hoveredInfo?.type !== type || hoveredInfo?.index !== index) {
      setHoveredInfo({ type, index });
    }
  };
  ...
  const handleDrop = (e, targetType, targetIndex) => {
    ...
    const payload = e.dataTransfer.getData("dragPayload");
    if (!payload) return;

    const { type: sourceType, index: sourceIndex } = JSON.parse(payload);

    if (sourceType === targetType && sourceIndex === targetIndex) return;

    // Helper to get fresh array copy
    const getList = (t) => {
          ...
          <div className="absolute top-[80px] left-0 right-0 z-40 bg-[#050807]/80 backdrop-blur-md px-8 py-3 border-b border-white/5 flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div className="bg-black/40 px-3 py-1 rounded text-xs text-white/50 border border-white/5 flex items-center gap-2">
                <span className="material-symbols-outlined text-sm">drag_indicator</span> Drag to Swap
              </div>
            </div>
            ...
            <div className="flex gap-6">
              <div className="flex flex-col items-end"><span className="text-[9px] font-bold text-white/40 uppercase">Rating</span><span className="text-xl text-yellow-400 italic">{squadRating}</span></div>
              <div className="flex flex-col items-end"><span className="text-[9px] font-bold text-white/40 uppercase">Chemistry</span><span className={`text-xl font-black italic ${parseInt(squadChemistry) >= 30 ? 'text-[#39ff14]' : 'text-yellow-400'}`}>{squadChemistry}</span></div>
            </div>
          </div>
          ...
          <div className="w-full h-full pt-[130px] grid grid-cols-1 lg:grid-cols-[1fr_380px] 2xl:grid-cols-[1fr_450px] gap-0">
            ...
            <div className="bg-[#121816]/95 border-l border-white/10 flex flex-col h-full overflow-hidden shadow-2xl z-30">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                ...
                <div>
                  <div className="flex justify-between items-center mb-3 px-1"><span className="text-xs font-bold text-white/40 uppercase tracking-wider">Reserves</span></div>
                  <div className="grid grid-cols-2 gap-3">
